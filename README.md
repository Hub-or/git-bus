...
```
（Git全局配置gitconfig路径：[磁盘或根目录路径]\\Users\\user\\.gitconfig)  
...
7.git log查看提交日志。  
8.git [命令] --help查看特定命令帮助。  
9.git help --all查看所有可执行命令。  
输入SHIFT + G跳转到达页面末尾，输入q退出阅览。  
10.git branch [分支名称]创建版本分支。  
11.git branch查看现有分支和当前分支。  
12.git checkout [分支名称]切换到分支名称对应分支。  
13.git merge [分支名称]合并目标分支到当前分支。  
...
8.git branch -m [当前名称] [目标名称]修改分支名称。  
9.git remote show origin显示origin对应云仓库仓库详细信息。  
10.git remote -v查看云仓库链接。  
11.git remote add [名称] [URL]增添云仓库链接。    
12.git fetch --prune根据云仓库分支裁剪本地仓库分支。  
13.git clone [URL]下载云仓库。  
14.git clone [URL] [文件夹路径]下载云仓库到路径对应文件夹。  
...
小贴士：考虑终端和文档文件，[]符号组合通常表示替换[]键位置填入[]中间文本对应场景文本面向程序需要场景文本替换适配场景程序需求。
